...
export const DB_PATH = process.env.GH_INBOX_DB || join(ROOT, 'data', 'inbox.sqlite');

/** The account whose inbox this is. Items authored by them are not "inbound work". */
export const OWNER_LOGIN = process.env.GH_INBOX_OWNER || 'jdx';

/** Search scope. `user:jdx` covers every repo the account owns. */
export const SEARCH_SCOPE = process.env.GH_INBOX_SCOPE || `user:${OWNER_LOGIN}`;

export const PORT = Number(process.env.GH_INBOX_PORT || 8787);
...
export function githubToken() {
  if (process.env.GH_INBOX_TOKEN) return process.env.GH_INBOX_TOKEN;
